...
class ClassRepository(BaseRepository[Class]):
    def __init__(self, session: AsyncSession):
        super().__init__(model=Class, session=session)
    ...
    async def list_scoped(
        ...
        if user_role == UserRole.ADMIN:
            ...
            cond = self.model.instructor_id == str(user_id)
            stmt = stmt.where(cond)
            count_stmt = count_stmt.where(cond)
        elif user_role == UserRole.STUDENT:
            student_exists = exists().where(
                Enrollment.class_id == self.model.id,
            ...
            # Agency or guest has no access to classes
