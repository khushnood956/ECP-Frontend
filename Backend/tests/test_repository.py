...
from app.repositories.params import PaginationParams, SortParams, FilterCondition, FilterOperator
...
@pytest.mark.asyncio
async def test_base_repository_create_and_get(db_session: AsyncSession, base_user: User):
...
@pytest.mark.asyncio
async def test_base_repository_update_and_delete(db_session: AsyncSession, base_user: User):
    repo = BaseRepository(StudentProfile, db_session)
    profile_id = str(uuid4())
...
async def test_base_repository_list(db_session: AsyncSession, base_user: User):
    ...
    p1 = StudentProfile(id=str(uuid4()), user_id=base_user.id, first_name="Alice", last_name="B")
...
async def test_transaction_commit(db_session: AsyncSession, base_user: User):
    ...
    async with tm.transaction():
        profile = StudentProfile(id=profile_id, user_id=base_user.id, first_name="Tx", last_name="Commit")
...
async def test_transaction_rollback(db_session: AsyncSession, base_user: User):
    ...
    try:
        async with tm.transaction():
            profile = StudentProfile(id=profile_id, user_id=base_user.id, first_name="Tx", last_name="Rollback")
            await repo.create(profile)
...
async def test_pagination_and_sorting(db_session: AsyncSession):
    ...
    users = []
    for i in range(25):
        u = User(id=str(uuid4()), email=f"test{i}@x.com", password_hash="h", role=UserRole.STUDENT, is_active=True, is_verified=True)
        users.append(u)
...
async def test_filtering(db_session: AsyncSession, base_user: User):
    repo = BaseRepository(StudentProfile, db_session)
    p1 = StudentProfile(id=str(uuid4()), user_id=base_user.id, first_name="Alice", last_name="Smith", city="NY")
    await repo.create(p1)
    
    # test EQ
    res = await repo.list_paginated(
        pagination=PaginationParams(page=1, page_size=10),
        filters=[FilterCondition(field="first_name", operator=FilterOperator.EQ, value="Alice")]
    )
    assert res.total == 1
    ...
    res2 = await repo.list_paginated(
        pagination=PaginationParams(page=1, page_size=10),
        filters=[FilterCondition(field="city", operator=FilterOperator.IN, value=["NY", "LA"])]
    )
    assert res2.total == 1
    ...
    res3 = await repo.list_paginated(
        pagination=PaginationParams(page=1, page_size=10),
        filters=[FilterCondition(field="last_name", operator=FilterOperator.LIKE, value="mit")]
    )
    assert res3.total == 1
    ...
    res4 = await repo.list_paginated(
        pagination=PaginationParams(page=1, page_size=10),
        filters=[FilterCondition(field="bio", operator=FilterOperator.IS_NULL, value=True)]
    )
    assert res4.total == 1
    ...
    with pytest.raises(RepositoryError):
        await repo.list_paginated(
            pagination=PaginationParams(page=1, page_size=10),
            filters=[FilterCondition(field="invalid_col", operator=FilterOperator.EQ, value="val")]
        )
...
async def test_bulk_operations(db_session: AsyncSession):
    ...
    users = [User(id=str(uuid4()), email=f"b{i}@x.com", password_hash="h", role=UserRole.STUDENT, is_active=True, is_verified=True) for i in range(3)]
    db_session.add_all(users)
    await db_session.flush()
    
    profiles = [
        StudentProfile(id=str(uuid4()), user_id=users[0].id, first_name="A", last_name="A"),
        StudentProfile(id=str(uuid4()), user_id=users[1].id, first_name="B", last_name="B"),
        StudentProfile(id=str(uuid4()), user_id=users[2].id, first_name="C", last_name="C"),
    ]
