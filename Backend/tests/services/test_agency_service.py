...
def agency_service(agency_repo_mock, mock_transaction_manager):
    return AgencyService(repository=agency_repo_mock, transaction_manager=mock_transaction_manager)

@pytest.mark.asyncio
async def test_verify_agency_twice_fails(agency_service, agency_repo_mock):
    agency_id = uuid.uuid4()
    mock_agency = Agency(id=agency_id, verification_status=AgencyVerificationStatus.VERIFIED)
...
async def test_suspend_agency_twice_fails(agency_service, agency_repo_mock):
    agency_id = uuid.uuid4()
    mock_agency = Agency(id=agency_id, verification_status=AgencyVerificationStatus.REJECTED)
    agency_repo_mock.get_by_id.return_value = mock_agency
