...
from app.models.enums import UserRole, DegreeLevel, FundingType, LeadStatus
...
async def test_domain_repositories_methods(db_session: AsyncSession):
    ...
    uid = str(uuid4())
    user = User(id=uid, email=f"{uid}@test.com", password_hash="hash", role=UserRole.STUDENT)
    ...
    profile = StudentProfile(id=str(uuid4()), user_id=uid, first_name="Test", last_name="Student")
    ...
    auid = str(uuid4())
    auser = User(id=auid, email=f"{auid}@agency.com", password_hash="hash", role=UserRole.AGENCY)
    ...
    sid = str(uuid4())
    sch = Scholarship(id=sid, title="Sch 1", country="USA", degree_level=DegreeLevel.BACHELOR, funding_type=FundingType.FULLY_FUNDED, is_active=True)
    await scholarship_repo.create(sch)
    
    sid2 = str(uuid4())
    sch2 = Scholarship(id=sid2, title="Sch 2", country="USA", degree_level=DegreeLevel.BACHELOR, funding_type=FundingType.FULLY_FUNDED, is_active=False)
    await scholarship_repo.create(sch2)

    active_sch = await scholarship_repo.get_active(PaginationParams(page=1, page_size=10))
    ...
    lid = str(uuid4())
    lead = Lead(id=lid, student_id=profile.id, agency_id=agency.id, scholarship_id=sid, status=LeadStatus.NEW)
...
async def test_cross_repository_transaction_commit(db_session: AsyncSession):
    ...
    async with tm.transaction():
        ...
        sch = Scholarship(id=sid, title="C", country="USA", degree_level=DegreeLevel.BACHELOR, funding_type=FundingType.FULLY_FUNDED)
