...
async def seed_database():
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            # Add Universities if they do not exist
            for u in [uni1, uni2, uni3]:
                result = await session.execute(select(University).where(University.name == u.name))
                if not result.scalar_one_or_none():
                    session.add(u)
                    print(f"Added University: {u.name}")
